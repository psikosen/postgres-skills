...
Resolve the repo first: `$PG_TOOLKIT_REPO` from `~/.claude/pg.env` (source it), else ask
the user where their `claude-code-setup-guide` clone lives — never guess.

## check — "am I current?"

```bash
cd "$PG_TOOLKIT_REPO/Postgres" && git pull --ff-only && ./install.sh --check
```
...
```bash
cd "$PG_TOOLKIT_REPO/Postgres" && git pull --ff-only && ./install.sh
```
